resource "azurerm_resource_group" "apis" {
  name     = "greengame-${var.locationShort}-apis-${random_integer.rg.result}rg"
...
resource "azurerm_app_service_plan" "product" {
  name                = "greengame-${var.locationShort}-apis-${random_integer.rg.result}-sp"
...
resource "azurerm_template_deployment" "backend" {
  name = "backendapistemplate-02"
  resource_group_name = "${azurerm_resource_group.apis.name}"
